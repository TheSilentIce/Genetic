...
// Format right now is to mutate, crossover, remutate, advance time, and then
...
void add_crossover() {
  ...
  for (i32 i = 0; i < population.size(); i++) {
    ...
    for (i32 j = 0; j < population.size(); j++) {
      ...
      Portfolio x =
          Portfolio(blend_crossover(&population[i], &population[j]), balance_);
    }
  }
}
void cull_population() {
  ...
  for (i32 i = population.size() - BASE_POPULATION; i < population.size();
       i++) {
